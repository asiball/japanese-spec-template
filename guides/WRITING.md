...
| 引用 | `> 引用文` | 左罫線+淡い背景 |
| 注記ボックス | `::: info` / `warning` / `error` 〜 `:::` | 種類名付きの色分けボックス |
...
情報・注意・エラーを本文から明確に分けたい場合は、Pandoc の fenced div 記法を使います。ボックス内では通常の Markdown 記法(太字・リスト・コードなど)を使えます。
...
`info` は青、`warning` は黄、`error` は赤を基調とし、色を識別できない場合でも分かるよう「情報」「注意」「エラー」の見出しも表示します。通常の引用は引き続き `> 引用文` を使い、注記ボックスと使い分けてください。

開始フェンスと閉じフェンス(`:::`)が対応していない原稿(閉じ忘れなど)は、pandoc が div として認識せず `:::` がそのまま本文に印字されるため、lint がエラーで停止します。種類名が上記 3 つ以外の場合は装飾なしのブロックになるため、lint が警告します。
